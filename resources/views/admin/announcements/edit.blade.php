...
<div class="mb-6 flex justify-between items-center">
    <div>
        <h2 class="text-2xl font-bold text-slate-800 font-outfit">Edit Pengumuman</h2>
        <p class="text-sm text-slate-500 mt-1">Ubah informasi atau pemberitahuan.</p>
    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-6">
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title', $announcement->title) }}" required class="block w-full rounded-xl border-slate-200 bg-slate-50 shadow-sm focus:border-teal-500 focus:ring focus:ring-teal-500/20 px-4 py-2.5">
                ...
                <div>
                    ...
                    <textarea name="content" rows="12" required class="block w-full rounded-xl border-slate-200 bg-slate-50 shadow-sm focus:border-teal-500 focus:ring focus:ring-teal-500/20 px-4 py-3">{{ old('content', $announcement->content) }}</textarea>
                ...
                <div class="bg-slate-50 p-5 rounded-xl border border-slate-100">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="is_published" class="block w-full rounded-lg border-slate-200 bg-white shadow-sm focus:border-teal-500 focus:ring focus:ring-teal-500/20 px-3 py-2">
                            <option value="1" {{ old('is_published', $announcement->is_published) == '1' ? 'selected' : '' }}>Publikasikan</option>
                            <option value="0" {{ old('is_published', $announcement->is_published) == '0' ? 'selected' : '' }}>Simpan sebagai Draft</option>
                        </select>
                    </div>

                    <div class="mb-4">
                        <label class="block text-sm font-medium text-slate-700 mb-2">Tanggal Publikasi</label>
                        <input type="datetime-local" name="published_at" value="{{ old('published_at', $announcement->published_at ? \Carbon\Carbon::parse($announcement->published_at)->format('Y-m-d\TH:i') : '') }}" class="block w-full rounded-lg border-slate-200 bg-white shadow-sm focus:border-teal-500 focus:ring focus:ring-teal-500/20 px-3 py-2">
                    </div>
                </div>
                ...
                <div class="bg-slate-50 p-5 rounded-xl border border-slate-100">
                    ...
                    <div>
                        ...
                        <div class="mb-3 relative group">
                            <img src="{{ Storage::url($announcement->thumbnail) }}" class="w-full h-32 object-cover rounded-lg border border-slate-200">
                        </div>
                        @endif
                        ...
                        <p class="text-xs text-slate-400 mt-2">Biarkan kosong jika tidak ingin mengubah lampiran.</p>
                    </div>
                </div>
