...
@section('title', 'Kategori Potensi Desa')

@section('content')
<div class="mb-6">
    <h2 class="text-2xl font-bold text-slate-800 font-outfit">Kategori Potensi</h2>
    <p class="text-sm text-slate-500 mt-1">Kelola kategori untuk pengelompokan UMKM dan potensi desa.</p>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-1">
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 sticky top-6">
            ...
            <form action="{{ route('admin.potential-categories.store') }}" method="POST">
                @csrf
                <div class="mb-4">
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Nama Kategori <span class="text-red-500">*</span></label>
                    <input type="text" name="name" value="{{ old('name') }}" required class="block w-full rounded-lg border-slate-200 bg-slate-50 shadow-sm focus:border-teal-500 focus:ring focus:ring-teal-500/20 px-4 py-2" placeholder="Cth: Kuliner">
                    ...
                    <tbody class="divide-y divide-slate-100 text-sm">
                        ...
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-right space-x-2">
                                ...
                                <form action="{{ route('admin.potential-categories.destroy', $item->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Hapus kategori ini? Kategori tidak bisa dihapus jika masih ada UMKM/Potensi yang menggunakannya.');">
                                    @csrf
                                    @method('DELETE')
                        ...
                        <tr>
                            <td colspan="3" class="px-6 py-12 text-center text-slate-500">
                                Belum ada data kategori potensi.
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
...
<div x-data="{ open: false, editId: null, editName: '' }" 
    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full">
            <form :action="'{{ url('admin/potential-categories') }}/' + editId" method="POST">
                @csrf
                @method('PUT')
