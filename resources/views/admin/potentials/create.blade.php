...
<div class="mb-6 flex justify-between items-center">
    <div>
        <h2 class="text-2xl font-bold text-slate-800 font-outfit">Tambah Potensi</h2>
        <p class="text-sm text-slate-500 mt-1">Tambahkan data UMKM atau potensi unggulan baru.</p>
    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" rows="15" required class="block w-full rounded-xl border-slate-200 bg-slate-50 shadow-sm focus:border-teal-500 focus:ring focus:ring-teal-500/20 px-4 py-3">{{ old('description') }}</textarea>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="potential_category_id" class="block w-full rounded-lg border-slate-200 bg-white shadow-sm focus:border-teal-500 focus:ring focus:ring-teal-500/20 px-3 py-2">
                            ...
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}" {{ old('potential_category_id') == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                            @endforeach
                        </select>
