...
@section('title', 'Tag Potensi Desa')

@section('content')
<div class="mb-6">
    <h2 class="text-2xl font-bold text-slate-800 font-outfit">Tag Potensi</h2>
    <p class="text-sm text-slate-500 mt-1">Kelola label/tag untuk mempermudah pencarian potensi desa dan UMKM.</p>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-1">
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 sticky top-6">
            <h3 class="font-bold text-lg text-slate-800 mb-4 font-outfit border-b border-slate-100 pb-3">Tambah Tag</h3>
            <form action="{{ route('admin.potential-tags.store') }}" method="POST">
                @csrf
                <div class="mb-4">
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Nama Tag <span class="text-red-500">*</span></label>
                    <input type="text" name="name" value="{{ old('name') }}" required class="block w-full rounded-lg border-slate-200 bg-slate-50 shadow-sm focus:border-teal-500 focus:ring focus:ring-teal-500/20 px-4 py-2" placeholder="Cth: Makanan Ringan">
                    ...
                    <tbody class="divide-y divide-slate-100 text-sm">
                        ...
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-right space-x-2">
                                <button onclick="editTag({{ $item->id }}, '{{ addslashes($item->name) }}')" class="inline-flex items-center p-2 bg-slate-100 hover:bg-teal-50 text-slate-600 hover:text-teal-600 rounded-lg transition-colors" title="Edit">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                </button>
                                <form action="{{ route('admin.potential-tags.destroy', $item->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Hapus tag ini?');">
                                    @csrf
                                    @method('DELETE')
...
<div x-data="{ open: false, editId: null, editName: '' }" 
    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full">
            <form :action="'{{ url('admin/potential-tags') }}/' + editId" method="POST">
                @csrf
                @method('PUT')
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <h3 class="text-lg font-bold text-slate-900 font-outfit mb-4 border-b border-slate-100 pb-2">Edit Tag</h3>
                    <div class="mb-4">
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Nama Tag</label>
