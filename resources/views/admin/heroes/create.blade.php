...
<div class="mb-6 flex justify-between items-center">
    <div>
        <h2 class="text-2xl font-bold text-slate-800 font-outfit">Tambah Hero Banner</h2>
        <p class="text-sm text-slate-500 mt-1">Buat gambar banner baru untuk halaman utama.</p>
    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-6">
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" class="block w-full rounded-xl border-slate-200 bg-slate-50 shadow-sm focus:border-teal-500 focus:ring focus:ring-teal-500/20 px-4 py-2.5">
                    @error('title') <p class="mt-1 text-sm text-red-500">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Subjudul Banner (Opsional)</label>
                    <textarea name="subtitle" rows="3" class="block w-full rounded-xl border-slate-200 bg-slate-50 shadow-sm focus:border-teal-500 focus:ring focus:ring-teal-500/20 px-4 py-3">{{ old('subtitle') }}</textarea>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="button_text" value="{{ old('button_text') }}" class="block w-full rounded-xl border-slate-200 bg-slate-50 shadow-sm focus:border-teal-500 focus:ring focus:ring-teal-500/20 px-4 py-2.5" placeholder="Contoh: Selengkapnya">
                    ...
                    <div>
                        ...
                        <input type="text" name="button_url" value="{{ old('button_url') }}" class="block w-full rounded-xl border-slate-200 bg-slate-50 shadow-sm focus:border-teal-500 focus:ring focus:ring-teal-500/20 px-4 py-2.5" placeholder="Contoh: https://...">
            ...
            <div class="space-y-6">
                <div class="bg-slate-50 p-5 rounded-xl border border-slate-100">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="sort_order" value="{{ old('sort_order', 1) }}" min="1" required class="block w-full rounded-lg border-slate-200 bg-white shadow-sm focus:border-teal-500 focus:ring focus:ring-teal-500/20 px-3 py-2">
                ...
                <div class="bg-slate-50 p-5 rounded-xl border border-slate-100">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-2">Upload Gambar <span class="text-red-500">*</span></label>
                        <input type="file" name="background_image" accept="image/*" required class="block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-teal-50 file:text-teal-700 hover:file:bg-teal-100 transition-colors">
                        @error('background_image') <p class="mt-1 text-sm text-red-500">{{ $message }}</p> @enderror
                        <p class="text-xs text-slate-400 mt-2">Rekomendasi rasio gambar 16:9 (Landscape) dengan resolusi minimal 1280x720.</p>
                    </div>
                </div>
