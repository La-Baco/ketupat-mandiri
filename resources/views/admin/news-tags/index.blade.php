...
@section('content')
<div class="mb-6">
    <h2 class="text-2xl font-bold text-slate-800 font-outfit">Tag Berita</h2>
    <p class="text-sm text-slate-500 mt-1">Kelola label/tag untuk mempermudah pencarian berita.</p>
</div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 sticky top-6">
            ...
            <form action="{{ route('admin.news-tags.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required class="block w-full rounded-lg border-slate-200 bg-slate-50 shadow-sm focus:border-teal-500 focus:ring focus:ring-teal-500/20 px-4 py-2" placeholder="Cth: Pembangunan">
                    ...
                    <tbody class="divide-y divide-slate-100 text-sm">
                        ...
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-right space-x-2">
                                ...
                                <form action="{{ route('admin.news-tags.destroy', $item->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Hapus tag ini?');">
                                    @csrf
                                    @method('DELETE')
...
<div x-data="{ open: false, editId: null, editName: '' }" 
    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full">
            <form :action="'{{ url('admin/news-tags') }}/' + editId" method="POST">
                @csrf
                @method('PUT')
